...
#For kenlm
top_k=4500000
input_txt_file_path="/path_to_corpus.txt"
output_path="/ouput_path"
kenlm_bins="/kenlm_build_path"
...
# 	python lm/concatenate_text.py --txt-files-dir ${txt_files_dir} --final-txt-file-save-path ${combined_txt_file_save_path}
# 	printf "*:File generated at: "${combined_txt_file_save_path}
# fi

if [ "$run_generate_lm_vocab" == 1 ]; then
	printf "\n** Generating kenlm **\n"
	python lm/generate_lm.py --input_txt ${input_txt_file_path} --output_dir ${output_path} \
		--top_k ${top_k} --kenlm_bins ${kenlm_bins} \
		--arpa_order 6 --max_arpa_memory "85%" --arpa_prune "0|0|1" \
...
# 	python lm/make_lexicon_lst.py --txt-file ${vocab_txt_file} --path-to-save-lexicon ${path_to_save_lexicon}
